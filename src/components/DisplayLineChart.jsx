...
export const DisplayLineChart = (props) => {
  ...
  //     return (
  //       <g key={i}>
  //         <circle cx={(800 / 3) * (i + 1)} cy={y0(value)} r="5" fill={color} />
  //         <line
  //           x1={(800 / 3) * i}
  //           y1={y0(arr[i - 1])}
  //           x2={(800 / 3) * (i + 1)}
    ...
    <div className="svgDisplay">
      <svg viewBox="0 0 1000 1000">
       ...
       return <g
        ...
        >
          {totalDataDept.map(item => item[janDeptIndex])
            .map((value, i, arr) => {
              let color, strokeWid;
...
//   console.log(
//     totalDataDept.map((item) => item[0]).map((value, i, arr) => arr[i])
//   );
